...
# First, try to load using dotenv
load_dotenv(override=True)

# Then, read the API key directly from the file
from utils.retry import retry_except
from tenacity import retry, stop_after_attempt, wait_fixed

system_message = "You are an AI trained to be a brilliant legal analyst."

@retry(stop=stop_after_attempt(3), wait=wait_fixed(2))
@retry_except(exceptions_to_catch=(IndexError, ZeroDivisionError), tries=3, delay=2)
def llm_call_gpt(GPT, input, system_p = None, temp = 0.7):
    client = OpenAI()
...
@retry(stop=stop_after_attempt(3), wait=wait_fixed(2))
@retry_except(exceptions_to_catch=(IndexError, ZeroDivisionError), tries=3, delay=2)
def llm_call_gpt_assistant(GPT, input, INSTRUCTION, temp = 0.7):
    client = OpenAI()
    client.api_key = os.getenv('OPENAI_API_KEY')
...
@retry(stop=stop_after_attempt(3), wait=wait_fixed(2))
@retry_except(exceptions_to_catch=(IndexError, ZeroDivisionError), tries=3, delay=2)
def llm_call_gpt_json(GPT, input, system_p = system_message, temp = 0.7):
    client = OpenAI()
    client.api_key = os.getenv('OPENAI_API_KEY')
    response = client.chat.completions.create(
        model=GPT,
...
@retry_except(exceptions_to_catch=(IndexError, ZeroDivisionError), tries=3, delay=2)
def llm_call_claude(LLM, input, system_p=None, temp=0.7):
    api_key = os.environ.get('ANTHROPIC_API_KEY')
    print(f"Debug: Full API key: {api_key}")  # Remove this line in production
...
@retry_except(exceptions_to_catch=(IndexError, ZeroDivisionError), tries=3, delay=2)
def llm_call_claude_json(LLM, input, system_p, temp = 0.7):
...
@retry_except(exceptions_to_catch=(IndexError, ZeroDivisionError), tries=3, delay=2)
def llm_call_groq(model, prompt, system_p = system_message, temp = 0.7):
...
@retry_except(exceptions_to_catch=(IndexError, ZeroDivisionError), tries=3, delay=2)
def llm_call_gemini(model, prompt, system_p=system_message):
